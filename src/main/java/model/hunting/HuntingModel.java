...
public class HuntingModel {
    ...
    private Map<String, List<HuntingStateActionPair>> pairsBySession;
    ...
    public Optional<Question> getBestAppropriateQuestion(
        ...
        Set<Question> validQuestions = new HashSet<>(graph.getAllAppropriateQuestions(gradeLevel, incorrectQuestions));
        ...
        HuntingStateActionPair bestStateAction = null;
        List<HuntingStateActionPair> explorableActions = new ArrayList<>();
        for( Question questionCandidate : validQuestions ) {
            // Make a state-action pair from the question
            HuntingStateActionPair candidateSAPair =
                    new HuntingStateActionPair(incorrectQuestions, questionCandidate);
    ...
    public void addReward(String sessionId, int stepsToGoal) {
        for( HuntingStateActionPair saPair : Lists.reverse(this.pairsBySession.get(sessionId)) ) {
            float newQValue =
    ...
    private float getMaxNextState(HuntingStateActionPair saPair) {
        List<Question> destinedState = new ArrayList<>(saPair.getState());
        destinedState.add(saPair.getAction());
        float nextReward = 0f;
        for( HuntingStateActionPair tablePair : this.qLearningTable.keySet() ) {
